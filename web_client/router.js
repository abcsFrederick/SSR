...
router.setQuery = function setQuery(name, value, options) {
    ...
    if (name === 'studyFolder') {
        delete queryString['filesystemFolder'];
        delete queryString['currentItem'];
    } else if (name === 'filesystemFolder') {
        delete queryString['studyFolder'];
        delete queryString['currentSeries'];
    }
