...
import { formatDate, formatSize, renderMarkdown, DATE_MINUTE, DATE_DAY, splitRoute, parseQueryString } from 'girder/misc';
...
import '../../stylesheets/widgets/collectionList.styl'
/**
    ...
    events: {
        'click a.g-collection-link': function (event) {
            ...
            if ($(event.delegateTarget).hasClass('g-ori-container')){
                router.navigate('ori_collection/' + this.collection.get(cid).id + unparsedQueryString, {trigger: true});
            }
            if ($(event.delegateTarget).hasClass('g-seg-container')){
                router.navigate('seg_collection/' + this.collection.get(cid).id + unparsedQueryString, {trigger: true});
            }
            if ($(event.delegateTarget).hasClass('selectionDom')){
                router.navigate('ds_collection/' + this.collection.get(cid).id + unparsedQueryString, {trigger: true});
            }
        },
